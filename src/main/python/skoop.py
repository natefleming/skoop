...
def parse_options(argv):
	...
	parser.add_argument('--conf', help='The location of the mappings configuration', required=True)
	parser.add_argument('--define', help='Define a property in the format of key=value', action=StoreKeyValue, dest='define', required=False)
	parser.add_argument('--log-level', help='The log level', choices=LOG_LEVELS,  default=DEFAULT_LOG_LEVEL, required=False)
	parser.add_argument('--dry-run', help='Perform a dry run', action='store_true', default=False, required=False)
...
def interpolate_value(value, context):
	...
	if isinstance(value, dict):
		return dict([(interpolate_value(k, context), interpolate_value(v, context)) for k, v in value.iteritems()])

	if isinstance(value, list):
		return [interopolate_value(v, context) for v in value]
	...
	while matcher.match('(.*)\${(.*)}(.*)'):
		...
		if ':' in target:
			...
			if len(components) != 2:
				raise ValueError('Defined actions must be in the form of action:argument')
			...
			if action == 'timestamp':
				...
				if argument not in context.properties:
					raise ValueError('Defined property {0} has not been provided'.format(argument))
				else:
	...
	def from_file(context, filename):
		...
		with open(filename, 'r') as f:
			try:
				...
				logger.exception('An exception has occurred loading file'.format(filename))
...
class Context(object):
	...
	@property
	def spark(self): return self._spark
...
class Environment(ModelBase):
	...
	@property
	def mappings(self): return sorted(self._mappings, key=lambda m: m.ordinal, reverse=True)
	...
	def __str__(self):
		return 'connections=[{0}], mappings=[{1}]'.format(self.connections, self.mappings)

	
class Mapping(ModelBase):

	def __init__(self, environment, name, data, defaults = {}):
		logger.info('Mapping(name={0}, data={1}, defaults={2})'.format(name, data, defaults))
		ModelBase.__init__(self, data, defaults)
		self._name = name
		self._source = Source(environment, data.get('source'), defaults.get('source', {}))
		self._destination = Destination(environment, data.get('destination'), defaults.get('destination', {}))
	...
	@property
	def ignore(self): return self._ignore

	@property
	def ordinal(self): return int(self._ordinal)

	def __str__(self):
		return 'name=[{0}], source=[{1}], destination=[{2}], ignore=[{3}, ordinal={4}]'.format(self.name, self.source, self.destination, self.ignore, self.ordinal)
...
class Datasource(ModelBase):
	...
	def __init__(self, environment, data, defaults = {}):
		...
		if any(c for c in environment.connections if c.name == connection_name):
			self._connection = next(c for c in environment.connections if c.name == connection_name)
		else:
			raise ValueError('Invalid connection name: {0}'.format(connection_name))

		self._num_connections = self.get_value('num-connections', DEFAULT_NUM_CONNECTIONS)
	...
	@property
	def num_connections(self): return self._num_connections

	def __str__(self):
		return 'Datasource(connection=[{0}], database=[{1}], table=[{2}])'.format(self.connection, self.database, self.table)
...
class Source(Datasource):
	...
	def __init__(self, environment, data, defaults = {}):
		...
		self._read_disposition = self.get_value('read-disposition', DEFAULT_READ_DISPOSITION)
		self._options = self.get_value('options', {})
		if not self._read_disposition.lower() in READ_DISPOSITIONS:
			raise ValueError('read-disposition must be in {0}'.format(READ_DISPOSITIONS))
	...
	@property
	def split_column(self): return self._split_column

	@property
	def lower_bound(self): return self._lower_bound

	@property
	def upper_bound(self): return self._upper_bound
	...
	@property
	def read_disposition(self): return self._read_disposition

	@property
	def include_columns(self): return [c.strip().lower() for c in self._include_columns]

	@property
	def exclude_columns(self): return [c.strip().lower() for c in self._exclude_columns]
...
class Destination(Datasource):
	...
	def __init__(self, environment, data, defaults = {}):
		...
		self._write_disposition = self.get_value('write-disposition', DEFAULT_WRITE_DISPOSITION)
		if not self._write_disposition.lower() in WRITE_DISPOSITIONS:
			raise ValueError('write-disposition must be in {0}'.format(WRITE_DISPOSITIONS))
		self._format = self.get_value('format', DEFAULT_DST_FORMAT)
		self._partition_by = self.get_value('partition-by', [])
		self._schema = self.get_value('schema')
		self._create_database = self.get_value('create-database', DEFAULT_CREATE_DATABASE)
		self._compute_statistics = self.get_value('compute-statistics', DEFAULT_COMPUTE_STATISTICS)
		self._options = self.get_value('options', {})
	...
	@property
	def create_backup(self): return str(self._create_backup).lower() == str(True).lower()

	@property
	def write_disposition(self): return self._write_disposition
	...
	@property
	def create_database(self): return str(self._create_database).lower() == str(True).lower()

	@property
	def compute_statistics(self): return str(self._compute_statistics).lower() == str(True).lower()
...
class Db2Reader(DatasourceReader):
	...
	def read(self, source):
		...
		if source.num_connections > 1:
			...
			if column:
				...
				reader = reader.option('numPartitions', source.num_connections) \
	...
	def _build_query(self, source, schema):
		if source.query:
			...
			if source.exclude_columns:
				columns = [c for c in columns if c.lower() not in source.exclude_columns]
			query = 'SELECT {0} FROM {1}.{2}'.format(','.join(columns), source.database, source.table)
		return query
	...
	def _load_schema(self, source):
		if source.query:
			...
			query='SELECT * FROM {0}.{1} WHERE 1 = 0'.format(source.database, source.table)
	...
	def _find_id_column(self, source, schema):
		...
		def rchop(s, suffix):
			...
			if s.endswith(suffix):
				result = s[:-len(suffix)]
			return result
		possible_ids = [f.name for f in schema.fields if rchop(f.name.lower(), '_id') in source.table]
		if possible_ids:
			possible_ids = [max(possible_ids, key=len)]
			logger.info('derived_from_name={0}'.format(possible_ids))
		else:
			first_column = schema.fields[0]
			if isinstance(first_column.dataType, T.IntegralType) and first_column.name.endswith('_id'):
	...
	def _find_upper_bound(self, source, column):
		logger.info('find_upper_bound(split_column={0},src={1})'.format(column, source.table))
		if source.upper_bound:
			return source.upper_bound
		if source.query:
			query='SELECT MAX({0}) AS id FROM ({1})'.format(column, source.query)
		else:
			query='SELECT MAX({0}) AS id FROM {1}.{2}'.format(column, source.database, source.table)
		result_set = self._jdbc_reader(source, query).load().collect()
	...
	def _find_lower_bound(self, source, column):
		logger.info('find_lower_bound(split_column={0},src={1})'.format(column, source.table))
		if source.lower_bound:
			return source.lower_bound
		if source.query:
			query='SELECT MIN({0}) AS id FROM ({1})'.format(column, source.query)
		else:
			query='SELECT MIN({0}) AS id FROM {1}.{2}'.format(column, source.database, source.table)
		result_set = self._jdbc_reader(source, query).load().collect()
...
class HiveReader(DatasourceReader):
	...
	def read(self, source):
		...
		query = source.query if source.query else 'SELECT * FROM {0}.{1}'.format(source.database, source.table)
...
class HiveWriter(DatasourceWriter):
    ...
    def write(self, df, destination):
        ...
        if destination.create_backup:
            if self._table_exists(destination):
                ...
                self.spark.sql('ALTER TABLE {0} RENAME TO {1}'.format(dst, backup_table))

		writer = writer.mode(destination.write_disposition).format(destination.format)
    ...
    def _table_exists(self, destination):
        ...
        logger.debug('Table {0}.{1}: {2}'.format(destination.database, destination.table, "Found" if found else "Not Found"))
        return found
	...
	def write(self, df, destination):
		...
		if destination.create_backup:
			if self._table_exists(hive, destination):
				...
				hive.executeUpdate('ALTER TABLE {0} RENAME TO {1}'.format(dst, backup_table))
...
class HiveWriter(DatasourceWriter):
	...
	def write(self, df, destination):
		logger.info('HiveWriter.write(destination={0})'.format(destination))
		if destination.create_database:
			self.spark.sql('CREATE DATABASE IF NOT EXISTS {0}'.format(destination.database))
		dst = '{0}.{1}'.format(destination.database, destination.table)
		if destination.create_backup:
			if self._table_exists(destination):
				backup_table = '{0}{1}'.format(dst, DEFAULT_BACKUP_SUFFIX)
...
class NullConnection(Connection):
	...
	def reader(self): return NullReader(self.environment.spark)
...
class JdbcConnection(Connection):
	...
	@property
	def jdbc_dialect(self): return self._jdbc_dialect_class
	...
	def register_dialect(self, spark):
		...
		self._jdbc_dialect = gw.jvm.java.lang.Class.forName(self._jdbc_dialect_class).newInstance()
		gw.jvm.org.apache.spark.sql.jdbc.JdbcDialects.registerDialect(self._jdbc_dialect)
	...
	def unregister_dialect(self, spark):
		...
		gw.jvm.org.apache.spark.sql.jdbc.JdbcDialects.unregisterDialect(self._jdbc_dialect)
...
class Db2Connection(JdbcConnection):
	...
	def reader(self): return Db2Reader(self.environment.spark)

	def writer(self): return Db2Writer(self.environment.spark)
...
class HiveConnection(HadoopConnection):
	...
	def reader(self): return HiveReader(self.environment.spark)

	def writer(self): return HiveWriter(self.environment.spark)
...
class Hive3Connection(HadoopConnection):
	...
	def reader(self): return Hive3Reader(self.environment.spark)

	def writer(self): return Hive3Writer(self.environment.spark)
...
class SparkVisitor(object):
	...
	class Task(object):
		...
		def __str__(self):
			return 'source=[{0}], destination=[{1}]'.format(self.source, self.destination)
